...
CAMINHO_ZIP_DIR = CAMINHO_RAIZ / 'arquivo01_diretorio'
CAMINHO_COMPACTADO = CAMINHO_RAIZ / 'arquivo01_compactado.zip'
CAMINHO_DESCOMPACTADO = CAMINHO_RAIZ / 'arquivo01_descompactado'

#Limpando caminhos (por segurança):
shutil.rmtree(CAMINHO_ZIP_DIR, ignore_errors=True) #4:
pathlib.Path.unlink(CAMINHO_COMPACTADO, missing_ok=True) #5:
shutil.rmtree(str(CAMINHO_COMPACTADO).replace('.zip', ''), ignore_errors=True) #6:
shutil.rmtree(CAMINHO_DESCOMPACTADO, ignore_errors=True)

#Criando diretório:
CAMINHO_ZIP_DIR.mkdir(exist_ok=True) #7:
...
with zipfile.ZipFile(CAMINHO_COMPACTADO, 'w') as zip_arquivo: #14:
    for root, dirs, files in os.walk(CAMINHO_ZIP_DIR): #15: #16:
        for indice2 in files:
            # print(indice2)
            zip_arquivo.write(os.path.join(root, indice2), indice2) #17:

#Lendo arquivos zip:
with zipfile.ZipFile(CAMINHO_COMPACTADO, 'r') as zip_arquivo:
    for indice3 in zip_arquivo.namelist(): #18:
        print(indice3)

#Descompactando/extraindo arquivos zip:
with zipfile.ZipFile(CAMINHO_COMPACTADO, 'r') as zip_arquivo:
    zip_arquivo.extractall(CAMINHO_DESCOMPACTADO) #19:

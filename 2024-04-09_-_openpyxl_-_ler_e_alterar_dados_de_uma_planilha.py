...
from openpyxl import Workbook, load_workbook #2: #23:
import openpyxl #3:
from openpyxl.worksheet.worksheet import Worksheet #4:

ROOT_FOLDER = Path(__file__).parent #5:
WORKBOOK_PATH = ROOT_FOLDER / 'workbook.xlsx' #6:

# workbook = Workbook() #7:
workbook: Workbook = load_workbook(WORKBOOK_PATH) #24:
# worksheet : Worksheet = workbook.active #8:
sheetname = 'LOG' #18:
# workbook.create_sheet(sheetname, 0) #19:
worksheet : Worksheet = workbook[sheetname]
for row in worksheet.iter_rows(): #25:
    for cell in row: #26:
        # print(cell) #27:
        print(cell.value, end='\t') #28:
        # if cell.value == 'Marie': #31:
        #     worksheet.cell(cell.row, 2, 100)
    print() #29:

worksheet['B3'].value = 50 #30:
...
#23: Imports the load_workbook module from openpyxl, which is used to load an existing Excel file.
#24: Loads the specified Excel file at WORKBOOK_PATH using the load_workbook method and assigns it to the workbook object.
#25: Iterates over each row in the worksheet using the iter_rows() method of the worksheet object.
#26: Iterates over each cell in a row.
#27: It would be to print the cell object.
#28: Prints the value of the current cell using cell.value and adds a tab (\t) to separate the values.
